...
function addRandomHyphen(input) {
    // Konversi string menjadi array karakter
    const chars = input.split('');

    // Tentukan posisi acak, tidak termasuk awal (0) dan akhir (chars.length)
    const randomIndex = Math.floor(Math.random() * (chars.length - 1)) + 1;
...
const generateRandomArray = (length) => Array.from({ length }, () => "ABCDEF"[Math.floor(Math.random() * 6)])
const generateMessageID = () => addRandomHyphen((0, crypto_1.randomBytes)(10).toString('hex').toUpperCase() + generateRandomArray(1)[0])
